(function($){
  ...
  if(window.location && window.location.pathname && window.location.pathname.toLowerCase() == '/redirection-aliexpress/') {
    var query = window.location.search.substring(1);
    var qs = parse_query_string(query);

    if(qs.id) {
      var link = "http://s.click.aliexpress.com/e/" + qs.id;
      $(".fallback-link").attr("href", link).css("display", "block");
